...
| QR route canonicalization | Verified | `/go/card` is a lightweight redirect to the canonical `/start` assessment. Express and the dedicated no-store Vercel redirect function add missing business-card/QR defaults while retaining incoming UTM and click parameters. The static HTML fallback applies the same browser behavior without maintaining a second assessment form. |
