# My PT Hub Onboarding â€” Setup & Test Guide
...
- Success page CTA button that links the customer directly to your My PT Hub access
- End-to-end metadata passthrough so the invite link travels from checkout creation â†’ Stripe â†’ success page â†’ email

## Configuration options

You can provide the My PT Hub access link in any of these ways (priority top to bottom):

1. Client param (recommended for flexibility)

- The pricing page now sends `myPtHubInvite` when creating the checkout session if present.
- You can set it via:
  - Add a meta tag on any page that launches checkout:
    `<meta name="mypthub:invite" content="https://link.mypthub.com/your-invite" />`
  - Or set a global in a script tag before the pricing JS loads:
    `window.GB_MYPTHUB_INVITE = 'https://link.mypthub.com/your-invite'`

1. Server environment fallback

- If no client param is provided, the server uses `MYPTHUB_INVITE_URL` from env.
- Set it in your environment:
  - `MYPTHUB_INVITE_URL=https://link.mypthub.com/your-invite`
...
If SMTP isnâ€™t fully configured, the email step is skipped safely.
