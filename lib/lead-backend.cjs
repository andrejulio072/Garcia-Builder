...
export const CANONICAL_ZAPIER_LEAD_KEYS = [
  'schemaVersion',
  'type',
  'lead_id',
  'submittedAt',
...
export const REQUIRED_CONSULTATION_FIELDS = [
  'firstName',
  'lastName',
  'email',
  'phone',
...
export const LEAD_DEDUP_WINDOW_MS = 10 * 60 * 1000;
...
export function getAliasedValue(source, names) {
...
export function validateCanonicalLead(payload) {
...
export function buildZapierLeadPayload(input = {}, options = {}) {
...
export function hasConsultationPayload(input = {}) {
  const source = input && typeof input === 'object' ? input : {};
  const aliases = [
...
export function isHotLead(payload) {
...
export function cleanupDuplicateCache(cache, now = Date.now()) {
...
export function reserveLeadId(cache, leadId, now = Date.now()) {
...
export function releaseLeadId(cache, leadId) {
  cache.delete(leadId);
}

export async function postJsonWithTimeout(url, payload, timeoutMs = 8000, fetchImpl = fetch) {
...
export async function forwardLeadToZapier(payload, {
  env = process.env,
  fetchImpl = fetch,
  logger = console,
  timeoutMs = 8000
...
export async function forwardHotLeadToZapier(payload, {
  env = process.env,
  fetchImpl = fetch,
  logger = console,
  timeoutMs = 8000
