...
function getEmailTimeoutMs(value = process.env.BREVO_TIMEOUT_MS) {
  const parsed = Number(value || DEFAULT_EMAIL_TIMEOUT_MS);
  if (!Number.isFinite(parsed)) return DEFAULT_EMAIL_TIMEOUT_MS;
  return Math.min(Math.max(Math.round(parsed), 1000), 20000);
}

async function fetchWithTimeout(fetchImpl, url, options, timeoutMs) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), getEmailTimeoutMs(timeoutMs));
  try {
...
async function sendWithBrevo({ to, subject, html, text, replyTo }, fetchImpl = fetch, timeoutMs) {
  ...
  const response = await fetchWithTimeout(fetchImpl, BREVO_API_URL, {
    method: 'POST',
    headers: {
  ...
  return { provider: 'brevo', messageId: data.messageId || data.messageIds?.[0] || null };
}
...
async function sendTransactionalEmail(options, adapters = {}) {
  ...
  if (brevoConfigured) {
    try {
      const result = await sendWithBrevo(options, fetchImpl, adapters.timeoutMs);
      return { ok: true, skipped: false, provider: result.provider, messageId: result.messageId || null };
    } catch (error) {
      if (!smtpConfigured) {
