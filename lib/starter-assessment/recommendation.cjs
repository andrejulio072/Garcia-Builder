...
function buildSummary(answers, language = 'en') {
  ...
  const extendedCopy = starterI18n.getSummaryCopy?.(lang);
  if (extendedCopy) {
    const goal = extendedCopy.goals?.[answers.primary_goal] || extendedCopy.goals?.['Not sure yet'];
    const translatedDays = starterI18n.translateText(answers.training_days, lang);
    const days = answers.training_days === 'I am unsure'
      ? extendedCopy.unsureDays
      : extendedCopy.trainingDays.replace('{days}', translatedDays);
    const barrier = starterI18n.translateText(answers.main_barrier || '', lang).toLocaleLowerCase(lang);
    return extendedCopy.template
      .replace('{goal}', goal)
      .replace('{days}', days)
...
function getSupportCTA(leadStatus, language = 'en') {
  ...
  const lang = starterI18n.normalizeLanguage(language);
  const copy = starterI18n.getSupportCTACopy?.(lang) || labels[lang] || labels.en;
