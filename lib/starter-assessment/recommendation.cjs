...
function buildSummary(answers, language = 'en') {
  ...
  if (lang !== 'en') {
    const goal = starterI18n.translateText(answers.primary_goal || 'Not sure yet', lang);
    const barrier = starterI18n.translateText(answers.main_barrier || 'Not sure yet', lang);
    const translatedDays = starterI18n.translateText(answers.training_days || 'I am unsure', lang);
    const templates = {
      fr: `D’après votre objectif « ${goal} », vos disponibilités (${translatedDays}) et votre difficulté principale (« ${barrier} »), le meilleur point de départ est une structure d’entraînement répétable associée à des habitudes nutritionnelles durables.`,
      de: `Ausgehend von deinem Ziel „${goal}“, deiner verfügbaren Trainingszeit (${translatedDays}) und deiner größten Hürde („${barrier}“) ist eine wiederholbare Trainingsstruktur mit nachhaltigen Ernährungsgewohnheiten der beste Startpunkt.`,
      it: `In base al tuo obiettivo “${goal}”, alla disponibilità (${translatedDays}) e all’ostacolo principale (“${barrier}”), il punto di partenza migliore è una struttura di allenamento ripetibile insieme ad abitudini nutrizionali sostenibili.`,
      nl: `Op basis van je doel ‘${goal}’, je beschikbare trainingsdagen (${translatedDays}) en je grootste obstakel (‘${barrier}’) is een herhaalbare trainingsstructuur met haalbare voedingsgewoonten het sterkste startpunt.`,
      pl: `Biorąc pod uwagę cel „${goal}”, dostępność (${translatedDays}) i główną przeszkodę („${barrier}”), najlepszym początkiem będzie powtarzalna struktura treningowa połączona z trwałymi nawykami żywieniowymi.`,
      ro: `Pe baza obiectivului „${goal}”, a disponibilității (${translatedDays}) și a obstacolului principal („${barrier}”), cel mai bun început este o structură de antrenament repetabilă, combinată cu obiceiuri alimentare sustenabile.`,
      ru: `С учётом вашей цели «${goal}», доступности (${translatedDays}) и главного препятствия («${barrier}») лучший старт — повторяемая структура тренировок в сочетании с устойчивыми привычками питания.`
    };
...
function getSupportCTA(leadStatus, language = 'en') {
  ...
  Object.assign(labels, {
    fr: { warm: 'Parler d’un plan personnalisé avec Andre', interested: 'Voir mes modèles recommandés', cold: 'Télécharger mon programme de 28 jours' },
    de: { warm: 'Mit Andre über einen individuellen Plan sprechen', interested: 'Meine empfohlenen Vorlagen ansehen', cold: 'Meinen 28-Tage-Start herunterladen' },
    it: { warm: 'Parla con Andre di un piano personalizzato', interested: 'Vedi i modelli consigliati', cold: 'Scarica il mio programma di 28 giorni' },
    nl: { warm: 'Bespreek een persoonlijk plan met Andre', interested: 'Mijn aanbevolen sjablonen bekijken', cold: 'Mijn 28-daagse kickstart downloaden' },
    pl: { warm: 'Porozmawiaj z Andre o indywidualnym planie', interested: 'Zobacz polecane szablony', cold: 'Pobierz 28-dniowy plan startowy' },
    ro: { warm: 'Discută cu Andre despre un plan personalizat', interested: 'Vezi modelele recomandate', cold: 'Descarcă programul de 28 de zile' },
    ru: { warm: 'Обсудить индивидуальный план с Andre', interested: 'Посмотреть рекомендованные шаблоны', cold: 'Скачать 28-дневный стартовый план' }
  });
  const copy = labels[starterI18n.normalizeLanguage(language)] || labels.en;
