...
function getBookingUrl() {
  return String(process.env.NEXT_PUBLIC_BOOKING_URL || process.env.BOOKING_URL || '').trim() || null;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { data: lead, error } = await supabase
      ...
      .select('id, result_token_expires_at, result_email_sent_at, primary_goal, desired_result, training_environment, training_days, main_barrier, nutrition_support, starting_timeline, support_preference, whatsapp, marketing_whatsapp_consent')
    ...
    return json(res, 200, {
      ok: true,
      recommendation: toVisitorRecommendation(recommendation),
      actions: {
        whatsappUrl: buildWhatsappUrl(answers),
        bookingUrl: getBookingUrl(),
        leadTemperatureCategory: recommendation.leadStatus,
        coachingUrl: '/online-coaching.html'
      },
      delivery: {
        emailSent: Boolean(lead.result_email_sent_at)
      }
    });
  } catch (error) {
