...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { data: lead, error } = await supabase
      ...
      .select('id, result_token_expires_at, language, primary_goal, desired_result, training_environment, training_days, main_barrier, nutrition_support, starting_timeline, support_preference, whatsapp')
    ...
    const language = req.query?.language
      ...
      : (lead.language || 'en');
    const recommendation = buildRecommendation(answers, { whatsapp: lead.whatsapp }, language);
