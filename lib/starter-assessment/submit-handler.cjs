...
function buildResultEmail({ req, contact, answers, recommendation, visitorRecommendation, resultUrl, contactActions }) {
  ...
  const planHtml = starterPlan.title ? `
      <div style="margin:22px 0;padding:18px;border:1px solid #e5e7eb;border-radius:8px;background:#f9fafb;">
        ...
        <h3 style="margin:16px 0 8px;font-size:17px;">Training this week: ${escapeHtml(training.title)}</h3>
        ...
        <h3 style="margin:16px 0 8px;font-size:17px;">Macro targets and simple diet: ${escapeHtml(nutrition.title)}</h3>
        ${macroHtml ? `<ul>${macroHtml}</ul>` : ''}
        ${mealHtml ? `<h4 style="margin:14px 0 6px;font-size:15px;">Simple day of eating</h4><ul>${mealHtml}</ul>` : ''}
        ${shoppingHtml ? `<h4 style="margin:14px 0 6px;font-size:15px;">Starter shopping list</h4><ul>${shoppingHtml}</ul>` : ''}
        <p><a href="${escapeHtml(toAbsoluteUrl(baseUrl, nutrition.calculatorUrl || '/nutrition-calculator.html'))}" style="color:#111827;font-weight:800;">Calculate exact calorie and macro targets</a></p>
      </div>` : '';
...
<html><body style="margin:0;background:#0b0d10;font-family:Arial,sans-serif;color:#111827;">
  <div style="max-width:640px;margin:0 auto;background:#ffffff;">
    ...
    <div style="padding:28px 24px;font-size:16px;line-height:1.6;">
      ...
      ${planHtml}
      <h2 style="font-size:20px;margin:22px 0 8px;">Helpful resources</h2>
      <ul>${resourceRows}</ul>
  ...
  return {
    ...
    text: [
      ...
      '',
      training.title ? `Training this week: ${training.title}` : '',
      ...
      '',
      nutrition.title ? `Macro targets and simple diet: ${nutrition.title}` : '',
