...
async function sendWarmLeadAlert({ contact, answers, recommendation, metadata }) {
  ...
  const leadAge = ageFromDob(contact.date_of_birth);
  const html = `<p><strong>Warm Garcia Builder Lead: ${escapeHtml(contact.first_name)}</strong></p>
    <ul>
      ...
      <li>Age: ${escapeHtml(leadAge ?? 'Not supplied')}</li>
      <li>Date of birth: ${escapeHtml(contact.date_of_birth || 'Not supplied')}</li>
  ...
  return sendTransactionalEmail({
    ...
    text: [
      ...
      `Age: ${leadAge ?? 'Not supplied'}`,
      `Date of birth: ${contact.date_of_birth || 'Not supplied'}`,
  ...
  tasks.push((async () => {
    try {
      const zapierResult = await notifyZapier({
        ...
        date_of_birth: validated.contact.date_of_birth || null,
  ...
  try {
    ...
    recentSubmissions.set(submissionKey, {
      ...
      response: {
        ...
        resourceDelivery: response.resourceDelivery,
