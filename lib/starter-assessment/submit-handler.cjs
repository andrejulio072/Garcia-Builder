...
async function verifyTurnstile(token, ip) {
  ...
  const testMode = !isProduction && (process.env.NODE_ENV === 'test' || process.env.TURNSTILE_TEST_MODE === 'true');
...
function getResultBasePath(metadata = {}) {
  const landingPath = normalizeText(metadata.landing_path, 300).toLowerCase();
  if (landingPath.startsWith('/assessment') || landingPath.startsWith('/starter-plan')) {
    return '/assessment';
  }
  return '/start';
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (insertError) {
      console.error('starter assessment insert failed', {
        code: insertError.code,
        message: insertError.message,
        email: maskEmail(validated.contact.email),
        phone: maskPhone(validated.contact.whatsapp)
      });
      return json(res, 500, { error: 'Unable to submit the assessment right now.' });
    }

    const resultUrl = `${getBaseUrl(req)}${getResultBasePath(validated.metadata)}/result/${token}`;
    ...
    if (recommendation.leadScore >= 8) {
      const warmLeadResult = await sendWarmLeadAlert({
        contact: validated.contact,
        answers: validated.answers,
        recommendation,
      ...
      hotLeadSent = warmLeadResult && warmLeadResult.ok === true;
    }
    ...
    const response = {
      ...
      resultToken: token,
      resultUrl,
      entryContext: validated.metadata.entry_context,
      leadTemperatureCategory: recommendation.leadStatus,
      ...
      leadSaved: true,
      zapierSent,
      emailSent: emailDelivery.status === 'sent',
      hotLeadSent,
