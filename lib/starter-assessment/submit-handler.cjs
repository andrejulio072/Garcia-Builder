...
function buildResultEmail({ req, contact, answers, recommendation, visitorRecommendation, resultUrl, contactActions, language }) {
  const normalizedLanguage = starterI18n.normalizeLanguage(language);
  const emailCopy = starterI18n.getEmailCopy(normalizedLanguage);
  const emailDirection = normalizedLanguage === 'ar' ? 'rtl' : 'ltr';
...
<html lang="${escapeHtml(normalizedLanguage)}" dir="${emailDirection}"><body style="margin:0;background:#0b0d10;font-family:Arial,sans-serif;color:#111827;text-align:${emailDirection === 'rtl' ? 'right' : 'left'};">
  ...
  return {
    ...
    text: [
      ...
      nutrition.calculatorUrl ? `${emailCopy.exactTargets || emailCopy.calculate}: ${toAbsoluteUrl(baseUrl, nutrition.calculatorUrl)}` : '',
      '',
      `${emailCopy.helpful}:`,
      ...resourcesForEmail
        .flatMap((resource) => [
      ...
      contactActions.whatsappUrl ? `${emailCopy.whatsappAndre || starterI18n.ui('messageAndre', normalizedLanguage)}: ${contactActions.whatsappUrl}` : '',
      contactActions.instagramUrl ? `Instagram: ${contactActions.instagramUrl}` : '',
      contactActions.bookingUrl ? `${emailCopy.bookConsultation || starterI18n.ui('bookConsultation', normalizedLanguage)}: ${contactActions.bookingUrl}` : '',
      contactActions.contactEmail ? `${emailCopy.emailAndre || starterI18n.ui('emailAndre', normalizedLanguage)}: ${contactActions.contactEmail}` : '',
      contactActions.siteUrl ? `${emailCopy.visitSite || starterI18n.ui('visitSite', normalizedLanguage)}: ${contactActions.siteUrl}` : '',
      '',
      `${emailCopy.privacy}: ${privacyUrl}`
    ].filter(Boolean).join('\n')
  };
}
...
async function sendWarmLeadAlert({ contact, answers, recommendation, metadata }) {
  ...
  const html = `<p><strong>Warm Garcia Builder Lead: ${escapeHtml(fullName)}</strong></p>
    <ul>
      <li>Name: ${escapeHtml(fullName)}</li>
      <li>Email: ${escapeHtml(contact.email)}</li>
      <li>Phone: ${escapeHtml(contact.phone || 'Not supplied')}</li>
      <li>Instagram: ${escapeHtml(contact.instagram || 'Not supplied')}</li>
      <li>Preferred contact: ${escapeHtml(contact.preferred_contact_method || 'Not supplied')}</li>
  ...
  return sendTransactionalEmail({
    to: alertEmail,
    subject: `Warm Garcia Builder Lead: ${fullName}`,
    html,
    text: [
      `Warm Garcia Builder Lead: ${fullName}`,
      `Name: ${fullName}`,
      `Email: ${contact.email}`,
      `Phone: ${contact.phone || 'Not supplied'}`,
      `Instagram: ${contact.instagram || 'Not supplied'}`,
      `Preferred contact: ${contact.preferred_contact_method || 'Not supplied'}`,
  ...
  tasks.push((async () => {
    try {
      const zapierResult = await notifyZapier({
        ...
        instagram: validated.contact.instagram || null,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const resultParams = new URLSearchParams({ lang: validated.language });
    if (String(validated.metadata.landing_path || '').startsWith('/go/card')) {
      resultParams.set('source', 'card');
    }
    const resultUrl = `${getBaseUrl(req)}/start/result/${token}?${resultParams.toString()}`;
