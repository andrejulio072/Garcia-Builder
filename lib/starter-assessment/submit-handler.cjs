...
const { generateResultTokenForSubmission, getTokenExpiryDate, hashResultToken } = require('./tokens.cjs');
...
const SUBMISSION_ID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
...
function getSubmissionId(body) {
  const candidate = String(body?.submission_id || body?.submissionId || '').trim().toLowerCase();
  return SUBMISSION_ID_RE.test(candidate) ? candidate : randomUUID();
}

function getResultTokenSecret() {
  const secret = process.env.RESULT_TOKEN_SECRET ||
    process.env.SUPABASE_SECRET_KEY ||
    process.env.SUPABASE_SERVICE_ROLE_KEY ||
    process.env.SUPABASE_SERVICE_KEY;
  if (!secret) throw new Error('Result-token secret is missing');
  return secret;
}
...
function buildResultEmail({ req, contact, answers, recommendation, visitorRecommendation, resultUrl, contactActions, language }) {
  ...
  const plansUrl = addEmailAttribution(toAbsoluteUrl(baseUrl, '/packages'), 'view_plans');
  const viewPlansLabel = starterI18n.ui('viewPlans', language) || 'View Coaching Plans';
  const workoutLibraryUrl = addEmailAttribution(toAbsoluteUrl(baseUrl, training.libraryUrl || '/workouts#workout-library'), 'workout_library');
  const macroCalculatorUrl = addEmailAttribution(toAbsoluteUrl(baseUrl, nutrition.calculatorUrl || '/nutrition-calculator'), 'macro_calculator');
  const quickStartHtml = `
      <div style="margin:22px 0;padding:18px;border:1px solid #f6c84e;border-radius:8px;background:#fff9e8;">
        ...
        <p style="margin:16px 0 0;">
          ...
          <a href="${escapeHtml(plansUrl)}" style="display:inline-block;margin:0 8px 8px 0;background:#f6c84e;color:#111827;text-decoration:none;font-weight:800;padding:12px 16px;border-radius:8px;">${escapeHtml(viewPlansLabel)}</a>
...
async function sendWarmLeadAlert({ contact, answers, recommendation, metadata }) {
  ...
  const html = `<p><strong>Warm Garcia Builder Lead: ${escapeHtml(contact.first_name)}</strong></p>
    <ul>
      <li>Full name: ${escapeHtml(contact.full_name || contact.first_name)}</li>
      <li>Email: ${escapeHtml(contact.email)}</li>
      <li>WhatsApp: ${escapeHtml(contact.whatsapp || 'Not supplied')}</li>
      <li>Age: ${escapeHtml(contact.age)}</li>
      <li>Instagram/Facebook profile: ${escapeHtml(contact.instagram_handle || 'Not supplied')}</li>
      <li>Facebook: ${escapeHtml(contact.facebook_profile || 'Not supplied')}</li>
      <li>Preferred contact: ${escapeHtml(contact.preferred_contact_method || 'No preference')}</li>
      <li>Best contact time: ${escapeHtml(contact.best_contact_time || 'Not supplied')}</li>
  ...
  return sendTransactionalEmail({
    ...
    text: [
      ...
      `Full name: ${contact.full_name || contact.first_name}`,
      `Email: ${contact.email}`,
      `WhatsApp: ${contact.whatsapp || 'Not supplied'}`,
      `Age: ${contact.age}`,
      `Instagram/Facebook profile: ${contact.instagram_handle || 'Not supplied'}`,
      `Facebook: ${contact.facebook_profile || 'Not supplied'}`,
      `Preferred contact: ${contact.preferred_contact_method || 'No preference'}`,
      `Best contact time: ${contact.best_contact_time || 'Not supplied'}`,
  ...
  tasks.push((async () => {
    try {
      const zapierResult = await notifyZapier({
        ...
        full_name: validated.contact.full_name,
        first_name: validated.contact.first_name,
        email: validated.contact.email,
        whatsapp: validated.contact.whatsapp || null,
        age: validated.contact.age,
        instagram_handle: validated.contact.instagram_handle || null,
        facebook_profile: validated.contact.facebook_profile || null,
        preferred_contact_method: validated.contact.preferred_contact_method || null,
        best_contact_time: validated.contact.best_contact_time || null,
        ...
        entry_context: validated.metadata.entry_context,
        ...
        latest_utm_source: validated.metadata.latest_utm_source,
        latest_utm_medium: validated.metadata.latest_utm_medium,
        latest_utm_campaign: validated.metadata.latest_utm_campaign,
        latest_utm_content: validated.metadata.latest_utm_content,
        latest_utm_term: validated.metadata.latest_utm_term,
        gclid: validated.metadata.gclid,
        gbraid: validated.metadata.gbraid,
        wbraid: validated.metadata.wbraid,
        fbclid: validated.metadata.fbclid,
        first_touch_at: validated.metadata.first_touch_at,
        latest_touch_at: validated.metadata.latest_touch_at,
        landing_url: validated.metadata.landing_url,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    validated.metadata.first_touch_at = validated.metadata.first_touch_at || submittedAt;
    validated.metadata.latest_touch_at = validated.metadata.latest_touch_at || submittedAt;
    ...
    const token = generateResultTokenForSubmission(submissionId, getResultTokenSecret());
    ...
    const insertPayload = {
      ...
      resource_acknowledgement_at: submittedAt,
      marketing_email_consent_at: validated.contact.marketing_email_consent ? submittedAt : null,
      ...validated.metadata,
    ...
    if (insertError) {
      if (insertError.code === '23505') {
        const { data: existingLead, error: lookupError } = await supabase
          .from('starter_assessment_leads')
          .select('id, created_at, event_id, result_token_expires_at')
        ...
        if (!lookupError && existingLead) {
          return json(res, 200, {
            ...
            resourceDelivery: { email: 'previously_requested' }
          });
        }
      }
...
// Exposed for isolated rendering/delivery checks. This does not create leads or
// trigger assessment side effects.
module.exports.buildResultEmail = buildResultEmail;
