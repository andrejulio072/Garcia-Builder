...
function buildResultEmail({ req, contact, answers, recommendation, visitorRecommendation, resultUrl, contactActions }) {
  ...
  const resourceRows = visitorRecommendation.resources.map((resource) => (
    `<li><strong>${escapeHtml(resource.requestedTitle || resource.title)}</strong>: ${escapeHtml(resource.description)}${Array.isArray(resource.details) && resource.details.length ? `<ul>${resource.details.map((detail) => `<li>${escapeHtml(detail)}</li>`).join('')}</ul>` : ''}</li>`
  )).join('');
...
<html><body style="margin:0;background:#0b0d10;font-family:Arial,sans-serif;color:#111827;">
  ...
  return {
    ...
    text: [
      ...
      '',
      ...visitorRecommendation.resources
        .filter((resource) => Array.isArray(resource.details) && resource.details.length)
        .flatMap((resource) => [
