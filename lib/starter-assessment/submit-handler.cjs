...
const { buildAssessmentZapierPayload, notifyZapier } = require('./zapier.cjs');
...
function buildResultEmail({ req, contact, answers, recommendation, visitorRecommendation, resultUrl, contactActions, language }) {
  ...
  const optionalButtons = [
    ...
    contactActions.contactUrl ? `<a href="${escapeHtml(addEmailAttribution(contactActions.contactUrl, 'contact_andre'))}" style="display:inline-block;margin:8px 0 0 0;padding:12px 16px;border-radius:8px;background:#f3f4f6;color:#111827;text-decoration:none;font-weight:700;">${escapeHtml(starterI18n.ui('contactAndreCta', language))}</a>` : ''
  ].join('');
...
async function sendWarmLeadAlert({ contact, answers, recommendation, metadata }) {
  ...
  const html = `<p><strong>Warm Garcia Builder Lead: ${escapeHtml(contact.first_name)}</strong></p>
    <ul>
      ...
      <li>Training environment: ${escapeHtml(answers.training_environment)}</li>
      <li>Training days: ${escapeHtml(answers.training_days)}</li>
      <li>Main barrier: ${escapeHtml(answers.main_barrier)}</li>
      <li>Nutrition support: ${escapeHtml(answers.nutrition_support)}</li>
  ...
  return sendTransactionalEmail({
    ...
    text: [
      ...
      `Training environment: ${answers.training_environment}`,
      `Training days: ${answers.training_days}`,
      `Main barrier: ${answers.main_barrier}`,
      `Nutrition support: ${answers.nutrition_support}`,
...
async function completeLeadSideEffects({ supabase, lead, validated, recommendation, resultUrl }) {
  ...
  tasks.push((async () => {
    try {
      const zapierPayload = buildAssessmentZapierPayload({
        lead,
        validated,
        recommendation,
        resultUrl
      });
      const zapierResult = await notifyZapier(zapierPayload);
      if (zapierResult.ok) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const resultUrl = `${getBaseUrl(req)}/start-result?token=${encodeURIComponent(token)}`;
