const crypto = require('crypto');

function generateResultToken() {
  return crypto.randomBytes(32).toString('base64url');
}

function generateResultTokenForSubmission(submissionId, secret) {
  const normalizedId = String(submissionId || '').trim().toLowerCase();
  const normalizedSecret = String(secret || '');
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(normalizedId)) {
    throw new Error('A valid submission ID is required for result-token generation');
  }
  if (normalizedSecret.length < 32) {
    throw new Error('Result-token secret must contain at least 32 characters');
  }
  return crypto
    .createHmac('sha256', normalizedSecret)
    .update(`starter-assessment-result:${normalizedId}`, 'utf8')
    .digest('base64url');
}

function hashResultToken(token) {
  return crypto.createHash('sha256').update(String(token || ''), 'utf8').digest('hex');
}

function getTokenExpiryDate(now = new Date()) {
  const days = Number.parseInt(process.env.RESULT_TOKEN_EXPIRY_DAYS || '30', 10);
  const safeDays = Number.isFinite(days) && days > 0 ? days : 30;
  return new Date(now.getTime() + safeDays * 24 * 60 * 60 * 1000);
}

module.exports = {
  generateResultToken,
  generateResultTokenForSubmission,
  getTokenExpiryDate,
  hashResultToken
};
