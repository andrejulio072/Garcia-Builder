...
const { classifyEntryContext } = require('./entry-context.cjs');
...
function validateMetadata(input) {
  const source = input && typeof input === 'object' ? input : {};
  const entryContext = normalizeText(source.entry_context, 40).toLowerCase();
  const requestedLanguage = normalizeText(source.language, 10).toLowerCase();
  const normalized = {
    language: ['en', 'pt', 'es'].includes(requestedLanguage) ? requestedLanguage : 'en',
    ...
    fbclid: normalizeText(source.fbclid, 160) || null,
    ...
    latest_gclid: normalizeText(source.latest_gclid, 120) || null,
    latest_gbraid: normalizeText(source.latest_gbraid, 120) || null,
    latest_wbraid: normalizeText(source.latest_wbraid, 120) || null,
    latest_fbclid: normalizeText(source.latest_fbclid, 160) || null
  };

  return {
    ...normalized,
    entry_context: ['qr', 'paid', 'organic'].includes(entryContext)
      ? entryContext
      : classifyEntryContext(normalized)
  };
}
