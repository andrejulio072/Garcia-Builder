...
function walk(dir) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (['node_modules', '.git', 'coverage', '.vercel', 'public', 'backups', 'tmp'].includes(entry.name)) continue;
...
for (const file of htmlFiles) {
  ...
  if (oldWording.test(visibleText)) failures.push(`${relative}: found old public wording`);
