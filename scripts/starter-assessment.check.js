...
assert(resultClient.includes('window.print()'), 'Result page should provide a print or PDF action');
assert(resultClient.includes('eatingDayIntro') && resultClient.includes('mealBuildGuide'), 'Simple day of eating should explain how to build each meal');
assert(resultClient.includes('starter-shopping-list'), 'Simple day of eating should keep a scannable shopping list');
...
assert.equal((paidAssessmentPage.match(/class="starter-transform-card(?:\s[^"]*)?"/g) || []).length, 5, 'Paid assessment should show five existing transformation examples');
assert(paidAssessmentPage.includes('/assets/images/transformations/conrad-before.jpg'), 'Paid assessment should show Conrad transformation evidence');
